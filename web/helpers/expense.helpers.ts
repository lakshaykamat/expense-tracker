...
export function validateExpenseData(data: {
  ...
  // Validate amount
  if (typeof data.amount !== 'number' || isNaN(data.amount) || data.amount < 0.01) {
  ...
  if (data.description !== undefined && data.description !== null) {
    ...
    if (trimmedDesc.length > 500) {
      return { valid: false, error: 'Description must be at most 500 characters long' };
    }
  }
  ...
  if (data.category !== undefined && data.category !== null) {
    ...
    if (trimmedCategory.length > 50) {
      return { valid: false, error: 'Category must be at most 50 characters long' };
    }
  }
